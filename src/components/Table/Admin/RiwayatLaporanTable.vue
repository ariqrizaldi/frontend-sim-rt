...
const items = computed(() => store.getters.getRiwayatLaporan)
...
const detailLaporan = ref({ tanggal: '', deskripsi: '' })
...
const clickDetail = (isModalActive, item) => {
  if (isModalActive) {
    detailLaporan.value.tanggal = item.id
    detailLaporan.value.deskripsi = item.hasil
  }
}
...
<template>
  ...
  >
    ...
    <p> {{ detailLaporan.tanggal }} </p>
    <divider />
    <div class="font-bold text-lg mb-2">
      Tindakan dari Kerusakan
    </div>
    <p> {{ detailLaporan.deskripsi }} </p>
    <divider />
  </modal-box>
  <modal-box
    ...
    <tbody>
      ...
      >
        ...
        <td data-label="Tanggal selesai">
          <small
            class="text-gray-500 dark:text-gray-400"
          >{{ convertDate(client.dateEnd) }}</small>
        </td>
        ...
        <td data-label="Detail">
          <jb-buttons
            v-if="detail.find( n => n.laporan == client.id)"
            type="justify-center"
            no-wrap
          >
            <jb-button
              ...
              :icon="mdiEye"
              small
              @click="clickDetail( isModalActive = true, detail.find( n => n.laporan == client.id))"
            />
          </jb-buttons>
          <small
            v-else
