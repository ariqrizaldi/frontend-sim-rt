...
const items = computed(() => store.getters.getJenisKendaraan)
...
const openModalEdit = (isModalActive, item) => {
  if (isModalActive) {
    ...
    form.maxPenumpang = item.maxPenumpang
  }
}
...
const update = () => {
  if (!v$.value.$error) {
    const data = {
      ...
      maxPenumpang: form.maxPenumpang
    }
    DataService.update('/jenisKendaraans/', form.id, data)
      .then(response => {
...
<template>
  <modal-box
    ...
    :large-title="'Edit Jenis Kendaraan #'+form.id"
    button="info"
    has-cancel
