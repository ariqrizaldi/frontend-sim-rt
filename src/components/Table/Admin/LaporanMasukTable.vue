...
const items = computed(() => store.getters.getLaporanKerusakanMasuk)
...
const rules = computed(() => {
  return {
    teknisi: { required: helpers.withMessage('Wajib diisi', required) }
  }
})
...
const submit = () => {
  if (!v$.value.$error) {
    ...
    DataService.create('/detailKerusakans', info)
      .then((response) => {
...
<template>
  ...
  >
    <field
      label="Pilih Teknisi"
      :error="v$.teknisi.$error"
      :message="v$.teknisi.$errors[0]"
    >
      <control
        v-model="form.teknisi"
        :options="teknisi"
      />
    </field>
  </modal-box>
  <modal-box
    ...
    :large-title="'Konfirmasi Tolak Laporan #'+idLaporanBatal"
    button="danger"
    has-cancel
    @submit.prevent="batal"
  >
    <p class="text-lg">
      Apakah anda yakin untuk tidak menindaklanjuti laporan ini?
    </p>
  </modal-box>
  <div
  ...
  <table>
    ...
    <tbody>
      ...
      >
        ...
        <td data-label="Pembuat Laporan">
          {{ pelapor.find( n => n.id == client.user).nama }}
        </td>
