...
const items = computed(() => store.getters.getUnitBarang)
...
const unitBarang = reactive({
  id: null,
  nama: ''
})

const openDeleteConfirm = (isModalActive, item) => {
  if (isModalActive) {
    unitBarang.nama = item.nama
    unitBarang.id = item.id
  }
}
...
<template>
  <modal-box
    ...
    :large-title="'Hapus Unit Barang - '+unitBarang.nama"
    button="danger"
    button-label="Yakin"
    has-cancel
    @submit.prevent="hapus"
  >
    <p class="text-lg">
        ...
        <td data-label="Aksi">
          ...
          >
            <jb-button
              ...
              tooltip="Hapus"
              small
              @click="openDeleteConfirm(isModalActive = true, client)"
            />
          </jb-buttons>
        </td>
      </tr>
